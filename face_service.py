...
from fr.core import verify_face_for_user

BROKER = "localhost"
PORT = 8883

SUB_TOPIC = "topic/fr/input"
PUB_TOPIC = "topic/fr/output"
...
def on_message(client, userdata, msg):
    ...
    userId    = data[3 + 16:3 + 16 + 16].decode('utf-8').rstrip('\x00')
    chunk = data[3 + 16 + 16:]
    ...
    try:
        fr_result = verify_face_for_user(img, userId)
        if fr_result:
            print("[FR]", f"{fr_result}#")
            client.publish(PUB_TOPIC, f"{qr_result}#{sessionId}#{userId}", qos=1)
